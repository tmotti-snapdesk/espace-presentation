...
export async function getAllPendingVisites(): Promise<PendingVisite[]> {
  ...
  try {
    ...
    for (const blob of blobs.filter((b) => b.pathname.endsWith(".json"))) {
      // The real fix against a stale copy is cacheControlMaxAge on the
      // put() below (Blob's CDN cache key ignores query strings, so a
      // `?t=` cache-buster alone would do nothing) — cache: "no-store"
      // here just keeps Next itself from also caching the response.
